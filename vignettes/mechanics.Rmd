...
These logs are different from other logs in that they are looking for specific
types of data, not valid or invalid values in the data. 
...
## text so we need to use validation status (no_change) from the logs to determine whether ## or not the item should be validated further.
##
##  `keep_validated_entries` inner_join the free_text_log with the existing_log and keeps validated entries.
## This allows us to identify which records to check with the multiple choice log.
## Similar accounting will need to be done with the "other_choice_log".
targets::tar_target(
  animal_owner_free_text_log_validated,
  ohcleandat::keep_validated_entries(existing_log = animal_owner_existing_log,
                         new_log = animal_owner_free_text_log)
...
targets::tar_target(
  ...
  ohcleandat::create_other_choice_log(
    ...
    corrected_free_text_log = animal_owner_free_text_log_validated
  )
),
