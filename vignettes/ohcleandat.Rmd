...
![](img/repo.png){width=75%}

The main folder structure within the repo is as follows:

 - **.github** – this folder contains yaml files for creating and testing the automations within Github actions.
 
 - **R** – this folder contains .R files for creating targets outside of the ohcleandat package. This could include writing rule scripts or pre-processing functions required to complete the cleaning and data validation process.

 - **auth** – this folder keeps token that stored as a json file. An example of a json file token used in this workflow is for pulling data from Google Drive.

 - **data** – this folder could be a place to store data files if your team desires but also is a place to temporary download data that gets used within the workflow. This folder should be encrypted if it will actually store data. See XXX for information about encrypting your repository. 

 - **outputs** – this folder to put temporary store data or validation log files that get created within the cleaning process and need to uploaded remotely to shared file system (ex. place export validation log in order to have a source to then upload the file to dropbox).
 
 - **renv** – this folder installed for the renv, a R package that helps you create reproducible environments for your R projects.

- **reports** – this folder stores integration validation reports.

- **dashboards** – this folder stores any dashboard created.
...
Within the [`{ohcleandat-template}`](https://github.com/One-Health-Research-Consulting/ohcleandat-repo-template#) as described above there are three main types of scripts that are used and described below:
 ...
 - **target workflow scripts** – naming system is "_targets.R" and live in the main working directory of the repository. These contain the targets that are named using nouns to describe the outputs of the targets. Within the target itself, functions are used to execute the different actions of the workflow. See a specific example of a target workflow script [`here`](https://github.com/One-Health-Research-Consulting/ohcleandat-repo-template/blob/main/_targets.R) from the ohcleandat-template repository.

 - **R function scripts** – these .R files live in the "R" folder. The naming system system uses a verb and then description naming system (ex. preprocessing_questionnare). Common functions written outside of the ohcleandat package are functions to get/pull raw data from it's original source, pre-processing to format data, and other specific cleaning functions outside the scope of the ohcleandat package.  See a specific example of an R function script [`here`](https://github.com/One-Health-Research-Consulting/ohcleandat-repo-template/blob/main/R/create_example_target.R) from the ohcleandat-template repository.
 
 - **rule scripts** – these .R files live in the "R" folder and are used to define the rules that will then be used in the targets to flag any errors or mistakes. These are written using the validate R package as described above. See a specific example of a rules script [`here`](https://github.com/One-Health-Research-Consulting/ohcleandat-repo-template/blob/main/R/create_example_rules.R) from the ohcleandat-template repository.
 
 - **integration markdown reports** – these .Rmd files live in the "reports" folder. These are used for the integration validations. These reports compare IDs that don’t match between data sets that are to be integrated as well as other cross-checks for data quality. See a specific example of a integration markdown script [`here`](https://github.com/One-Health-Research-Consulting/ohcleandat-repo-template/blob/main/reports/integration_report_example.Rmd) from the ohcleandat-template repository.
...
## Getting Connected: Credentials and Access Setup

This project uses GitHub Actions to automatically pull data from our source systems and store it in the designated storage location. Before the automations can run, a few pieces need to be in place:

1. **API tokens for your data sources:** Each tool we pull from (e.g., REDCap, ODK) requires its own API token. Request a token from your project administrator or generate one through your account settings in that tool. Tokens are tied to your individual access level, so make sure your account has export permissions for the relevant project.

2. **Repository secrets:**  API tokens and credentials are never written directly into code. Instead, they're stored as encrypted secrets in the [`GitHub repository`](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-secrets) (Settings → Secrets and variables → Actions). The automation workflows reference these secrets by name at runtime. If you're adding a new data source, add its token as a repository secret and use the same naming convention as the existing entries.

3. **Storage access:** The workflows are set to write output to DropBox. Confirm the automation's service account or token has write permissions to this location.

Once these are in place, the workflows will run on their defined schedule with no further action needed. See the next section for how to verify a successful run.

## Recommended Targets Workflow:

This workflow uses the [`targets`](http://books.ropensci.org/targets/) package to manage the data cleaning and validation process. Rather than running scripts manually in sequence, targets defines the entire workflow as a pipeline of interdependent steps, each producing a defined output (a "target").

The pipeline is specified in _targets.R files, where each target calls a function from the project's function library (stored in the R/ folder). The core flow looks like this:

1. Pull raw data
2. pre-edit
3. read in existing validation log
4. Clean data
5. Cleaning checks
